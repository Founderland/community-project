	...
	const save = () => {
		...
		if (Object.values(data).every((value) => value.length > 0)) {
			if (password !== confirmPassword) {
				setIModal(true)
				setModalMessage({ icon: 'info', title: 'Validation error', message: 'Both passwords must match' })
			} else {
				console.log(data)
			...
			setIModal(true)
			setModalMessage({ icon: 'info', title: 'Validation error', message: 'All fields are required' })
		}
	}
