...
export default function DisplayResponse(props) {
  ...
  useEffect(() => {
    ...
    if (props.data.answerList && props.data.answerList?.length > 0) {
      ...
      for (const item of answerlist) {
        // console.log(catList)
        // console.log(catList.findIndex(x => x === item.question_category))
  ...
  const answerItem = (item) => (
    ...
    <div className='bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 '>
      ...
      <div>
      <dd className=" hidden  md:flex mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"> ({item.answer_rank} )</dd>
      </div>
      </div>
      </>
  ); 
