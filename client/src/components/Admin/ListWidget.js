					...
					<tbody className="text-gray-600 text-sm font-light">
						{dataToDisplay?.length ? (
									...
									if (header.key !== '-') {
										if (Array.isArray(item[header.key])) {
											return (
												<td
													key={item[header.key]}
													className={`py-3 px-6 ${header.style}`}
												>
							...
							<tr className="border-b border-gray-200 hover:bg-gray-100">
								<td colSpan={data.header.length} className="py-3 px-6 text-left">
					...
					<div className="border-b min-w-max w-full border-gray-200">
						<div className="flex items-center justify-center">
							<ReactPaginate
								...
								containerClassName={'my-1 flex py-1 list-none outline-none text-mono'}
								pageClassName={
