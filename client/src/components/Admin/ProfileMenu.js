	...
	return (
		<Menu as="div" className="relative">
			<Menu.Button className="flex items-center space-x-3 relative focus:outline-none">
				<h2 className="text-gray-800 text-bold text-sm hidden sm:block">{user.name}</h2>
				<img className="h-9 w-9 rounded-full border-2 border-fblue object-cover" src={user.avatar} alt={user.name} />
			</Menu.Button>
			<Transition
				as={Fragment}
