...
export default function Sidebar(props) {
  ...
  useEffect(() => {
    ...
    const fiteredArray = memberDetails.filter((items) => {
      return props.data?.lat === items.geoLocation.lat && props.data?.lng === items.geoLocation.lng;
    });
    ...
    <div>
      <div className='  absolute right-0 top-0 bottom-0 pb-3 z-50 m-2 md:mr-4 '>
        {(!isCardSelected && isSidebarSelected && !isNameSelected ) ? (
          <div className=' w-full md:w-10/12 md:w-full h-full max-w-2xl px-5 py-6 mt-2 bg-gray-900 opacity-90  shadow flex flex-col items-evenly'>
           ...
           {sidebarDisplay[0]?.city.toUpperCase()}{" "}
            ...
            {sidebarDisplay?.map((item) => (
              <div class='flex flex-col items-center md:items-stretch md:flex md:flex-row   max-w-md bg-white shadow-lg rounded-sm overflow-hidden m-5'>
                <div class='w-1/2 md:w-1/3 flex flex-col mt-2 md:mt-0 '>
                  <img
                ...
                <div className='w-3/4 md:w-2/3 p-4'>
                  ...
                  <h1 class='text-gray-900 font-bold text-lg md:text-xl lg:text-2xl 2xl:text-3xl '>
          ...
          //     </div>
          //     <div class='px-6 py-4'>
          //       <div class='font-bold text-lg md:text-xl mb-2'>{(selectedMember?.companyname)?.toUpperCase()}</div>
          //       <p class='text-gray-900 text-base md:text-xl'>
          //    {selectedMember.city}
          //         </p>
